#!/usr/bin/env python3
"""Exploratory D4.5 successor fixed for downstream D5 development.

The successor adopts the one isolated traversal repair supported by the
prior causal and generalization evaluations.  Relative to published D4.5
0.1.0, it changes only the enumeration of candidate consecutive-frame pairs:
...
signal-weighted centroid construction, and repeat certification remain
byte-for-byte imported from ``d45_aggregation.py``.  This module makes no
lattice decision and consumes no cell, orientation, processed reflection, or
conventional indexing result.
"""
...
def aggregate_d45_successor(
    q: np.ndarray,
    radius: np.ndarray,
    peaks: list[dict[str, Any]],
    frames: dict[str, dict[str, Any]],
) -> tuple[np.ndarray, list[dict[str, Any]], dict[str, Any]]:
    """Construct the fixed exploratory D4.5 successor representation."""
    ...
    construction_record = {
        "construction": (
            ...
            "REPEAT_CERTIFIED_CENTROID_CONSOLIDATION"
        ),
        "successor_status": (
            "FIXED_EXPLORATORY_D45_INPUT_REPRESENTATION_FOR_D5_"
            "SUCCESSOR_DEVELOPMENT_NOT_A_CONFIRMATORY_METHOD"
        ),
